...
fun Notifications (match: MatchDomain, onClick: NotificationOnClick) {
    Row (
        ...
        val drawlable = if (match.notificationEnabled) R.drawable.ic_notifications_active
        else R.drawable.ic_notifications

        Image(
            painter = painterResource(id =drawlable),
...
fun Teams (match: MatchDomain){
    Row (
        ...
        Text(
            text = "X",
            modifier = Modifier.padding(end = 19.dp, start = 16.dp),
            style = MaterialTheme.typography.h6.copy(color = Color.White)
            )
